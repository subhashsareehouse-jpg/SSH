...
# ---------------------------
# OTP AUTH DATABASE (tmpDurgaOTPConfig)
# ---------------------------
SQL_CONN_STR_AUTH = (
    "DRIVER={SQL Server};"
    "SERVER=localhost\\SQL2022;"
    "DATABASE=BusyComp0001_db;"   # <-- IMPORTANT (NOT db12025)
    "UID=SA;"
    "PWD=busy123"
)
...
def send_plain_text(to: str, body: str):
    ...
    try:
        ...
        try:
            ...
            jr = {"raw": r.text}
        app.logger.info(f"[BSP/TEXT] ↩ {r.status_code}")
        return r.status_code, jr
    except Exception as e:
        app.logger.exception("[BSP/TEXT] request failed")
        return 500, {"error": {"message": str(e)}}
# ---------------------------
# OTP WhatsApp Sender
# ---------------------------
def send_wa_otp(msisdn_91, otp):
    msg = f"Your Busy Login OTP is: {otp}\nValid for 5 minutes.\n- Subhash Saree House"
    sc, jr = send_plain_text(msisdn_91, msg)
    return 200 <= int(sc) < 300

app.config["SEND_WA_OTP"] = send_wa_otp
...
def job_gmail_to_whatsapp():
    try:
        if fetch_new_emails is None:
            app.logger.error("[gmail->wa] fetch_new_emails import failed. Check gmail_dump_to_whatsapp.py path.")
            return
        ...
        for item in items:
            ...
            # ✅ send message + attachments to group
            wa_send_group_internal(GMAIL_TO_WA_GROUP_ID, text=msg, attachments=atts)
            sent += 1

        app.logger.info(f"[gmail->wa] done | sent={sent}")

    except Exception:
        app.logger.exception("[gmail->wa] job failed")


# ✅ IMPORTANT: avoid double scheduler in debug/reloader (and in production run normally)
def _register_gmail_scheduler_job():
    try:
        scheduler.add_job(
            job_gmail_to_whatsapp,
            "interval",
        ...
        app.logger.info("[gmail->wa] scheduler job registered (every 10 minutes)")
    except Exception:
        app.logger.exception("[gmail->wa] scheduler add_job failed")


if os.environ.get("WERKZEUG_RUN_MAIN") == "true" or not app.debug:
    _register_gmail_scheduler_job()
