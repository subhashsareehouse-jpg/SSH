...
# ================================
# Persistent WhatsApp Outbox (Disk)
# ================================
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
OUTBOX_DIR = os.path.join(BASE_DIR, "wa_outbox")
OUTBOX_SENT_DIR = os.path.join(OUTBOX_DIR, "_sent")
OUTBOX_FAIL_DIR = os.path.join(OUTBOX_DIR, "_failed")
os.makedirs(OUTBOX_DIR, exist_ok=True)
os.makedirs(OUTBOX_SENT_DIR, exist_ok=True)
os.makedirs(OUTBOX_FAIL_DIR, exist_ok=True)

SEND_STATE_FILE = os.path.join(OUTBOX_DIR, "send_state.json")
SEND_STATE_LOCK = threading.Lock()

def _load_send_state() -> dict:
    if not os.path.exists(SEND_STATE_FILE):
        return {"rr_index": 0, "session_next_ts": {}, "last_by_number": {}}
    try:
...
def _save_send_state(st: dict) -> None:
    tmp = SEND_STATE_FILE + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(st, f, ensure_ascii=False, indent=2)
    os.replace(tmp, SEND_STATE_FILE)

def _normalize_allowed_sessions(val: str) -> list[str]:
    s = (val or "").strip()
    if not s or s.upper() == "ALL":
        return [x["senderId"] for x in FALLBACK_SESSIONS]
    return [p.strip() for p in s.split(",") if p.strip()]

def _pick_round_robin_sender(allowed: list[str]) -> str | None:
    if not allowed:
        return None
    with SEND_STATE_LOCK:
        st = _load_send_state()
        idx = int(st.get("rr_index") or 0)
        sid = allowed[idx % len(allowed)]
        st["rr_index"] = (idx + 1) % 10_000_000
        _save_send_state(st)
    return sid

def _get_next_ts(key: str) -> float:
    with SEND_STATE_LOCK:
        st = _load_send_state()
        return float((st.get("session_next_ts", {}) or {}).get(str(key), 0))

def _set_next_ts(key: str, ts: float) -> None:
    with SEND_STATE_LOCK:
        st = _load_send_state()
        m = st.get("session_next_ts", {}) or {}
        m[str(key)] = float(ts)
        st["session_next_ts"] = m
        _save_send_state(st)

def _mark_last_sent(to_number: str, sender_key: str) -> None:
    with SEND_STATE_LOCK:
        st = _load_send_state()
        last = st.get("last_by_number", {}) or {}
        last[str(to_number)] = {"ts": time.time(), "sender": str(sender_key)}
        st["last_by_number"] = last
        _save_send_state(st)
...
# -------------------------------------------------------
# WhatsApp Template Registry (templates_registry.xlsx)
# -------------------------------------------------------
_template_registry = {}
...
def load_template_registry():
    ...
    try:
        if os.path.exists(REGISTRY_XLSX):
            ...
            ft = cols.get("fallback_text") or "fallback_text"

            # NEW columns (your sheet)
            asess = cols.get("allowed_sessions") or "allowed_sessions"
            gnmin = cols.get("gap_nor_min") or cols.get("gap_normal_min_sec") or "gap_nor_min"
            gnmax = cols.get("gap_nor_max") or cols.get("gap_normal_max_sec") or "gap_nor_max"
            bwsec = cols.get("burst_window") or cols.get("burst_window_sec") or "burst_window"
            gbmin = cols.get("gap_burs_min") or cols.get("gap_burst_min_sec") or "gap_burs_min"
            gbmax = cols.get("gap_burs_max") or cols.get("gap_burst_max_sec") or "gap_burs_max"
            lf1 = cols.get("local_file_1") or cols.get("local_file") or "local_file"
            lf2 = cols.get("local_file_2") or "local_file_2"
            bspcol = cols.get("bsp_enable") or "bsp_enable"
            ...
            for _, r in df.iterrows():
                ...
                reg[name.lower()] = {
                    ...
                    "vars": _to_int(r.get(vc, 0), 0),
                    "default_media_link": str(r.get(dl, "")).strip(),
                    "fallback_text": str(r.get(ft, "")).strip(),

                    # NEW
                    "allowed_sessions": str(r.get(asess, "")).strip(),   # e.g. "9179...,9182..." or "ALL"
                    "gap_normal_min_sec": _to_int(r.get(gnmin, 50), 50),
                    "gap_normal_max_sec": _to_int(r.get(gnmax, 70), 70),
                    "burst_window_sec": _to_int(r.get(bwsec, 120), 120),    # you used 90; ok
                    "gap_burst_min_sec": _to_int(r.get(gbmin, 5), 5),
                    "gap_burst_max_sec": _to_int(r.get(gbmax, 8), 8),
                    "local_file_1": str(r.get(lf1, "")).strip(),
                    "local_file_2": str(r.get(lf2, "")).strip(),
                    "bsp_enable": _is_on(r.get(bspcol, "")),  # ON/OFF
                }
...
FALLBACK_SENDER_IDS = _csv_list(os.getenv("FALLBACK_SENDER_IDS", ""))
FALLBACK_AUTH_TOKENS = _csv_list(os.getenv("FALLBACK_AUTH_TOKENS", ""))

FALLBACK_SESSIONS = []
if FALLBACK_SENDER_IDS and FALLBACK_AUTH_TOKENS and len(FALLBACK_SENDER_IDS) == len(FALLBACK_AUTH_TOKENS):
    for sid, tok in zip(FALLBACK_SENDER_IDS, FALLBACK_AUTH_TOKENS):
        FALLBACK_SESSIONS.append({"senderId": sid, "authToken": tok})
else:
    # backward compatible (if old single vars exist)
    _sid = (os.getenv("FALLBACK_SENDER_ID", "") or "").strip()
    _tok = (os.getenv("FALLBACK_AUTH_TOKEN", "") or "").strip()
    if _sid and _tok:
        FALLBACK_SESSIONS.append({"senderId": _sid, "authToken": _tok})

def _fallback_can_send() -> bool:
    return bool(FALLBACK_ENABLE and FALLBACK_API_URL and FALLBACK_SESSIONS)

def get_fallback_session(sender_id: str) -> dict | None:
    for s in FALLBACK_SESSIONS:
        if str(s.get("senderId")) == str(sender_id):
            return s
    return None
def _fallback_can_send() -> bool:
...
def send_fallback_botmaster(
    ...
    """
    Send via BotMasterSender.

    ✅ Supports multi-session rotation:
        session = {"senderId": "...", "authToken": "..."}
        If session is None -> uses first session from FALLBACK_SESSIONS,
        else falls back to old FALLBACK_SENDER_ID / FALLBACK_AUTH_TOKEN.

    ✅ Prefers direct file upload if local_path is provided,
       otherwise falls back to mediaurl, otherwise plain text.
    """
    if DRY_RUN:
        app.logger.info(
            f"[FALLBACK] (dry-run) to={receiver_id} text~={text[:80]} media={bool(media_url or local_path)}"
        )
        return 200, {"dry_run": True}

    # -----------------------------
    # Pick senderId/authToken
    # -----------------------------
    sender_id = ""
    auth_token = ""

    if session and isinstance(session, dict):
        sender_id = str(session.get("senderId", "")).strip()
        auth_token = str(session.get("authToken", "")).strip()

    # If caller didn't pass session, use pool (rotation manager will pass session normally)
    if (not sender_id or not auth_token):
        try:
            if "FALLBACK_SESSIONS" in globals() and FALLBACK_SESSIONS:
                sender_id = str(FALLBACK_SESSIONS[0].get("senderId", "")).strip()
                auth_token = str(FALLBACK_SESSIONS[0].get("authToken", "")).strip()
        except Exception:
            pass

    # Backward compatible (old single vars)
    if not sender_id:
        sender_id = str(globals().get("FALLBACK_SENDER_ID", "")).strip()
    if not auth_token:
        auth_token = str(globals().get("FALLBACK_AUTH_TOKEN", "")).strip()

    # Base payload
    payload = {
        "senderId": str(sender_id),
        "receiverId": str(receiver_id),
        "messageText": str(text or ""),
    ...
    try:
        ...
        app.logger.info(
            f"[FALLBACK] sender={sender_id} ↩ {r.status_code} body~={str(jr)[:160]}"
        )
...
def _make_bsp_headers():
    ...
    return headers
def is_bsp_enabled_for_template(tpl_name: str) -> bool:
    info = _template_registry.get((tpl_name or "").strip().lower(), {}) or {}
    return bool(info.get("bsp_enable", False))  # default OFF unless ON in Excel
...
def enqueue_send_job(
    ...
    """
    Queue (or schedule-delayed) a send job.

    NEW BEHAVIOR:
      ✅ Per-template BSP ON/OFF from Excel column 'bsp_enable' (ON/OFF)
      ✅ BSP tried first ONLY when bsp_enable=ON (and GLOBAL_FORCE_FALLBACK_ALL is False)
      ✅ If BSP is on cooldown OR template BSP OFF OR BSP fails → fallback immediately
      ✅ Fallback uses session rotation + per-session delay (50–70 sec) and burst 5–8 sec
      ✅ Persist last-sent sender per number + cooldown in send_state.json (restart safe)
      ✅ Optional template attachments: local_file_1/local_file_2 (skip if empty)
    """
    ...
    # Decide media for fallback (priority: pdf_link > image_link > video_link > pdf_path upload)
    ...
    if pdf_link:
        ...
        local_path_for_fb = pdf_path  # direct file upload in BotMaster

    # ---- template info from registry ----
    tinfo = _template_registry.get((template_name or "").strip().lower(), {}) or {}

    # BSP enable from Excel (ON/OFF) - default OFF if missing
    tpl_bsp_enable = bool(tinfo.get("bsp_enable", False))

    # Allowed fallback sessions from Excel
    allowed_sessions_raw = str(tinfo.get("allowed_sessions", "")).strip() or "ALL"

    # Delay params from Excel (with safe defaults)
    gn_min = int(tinfo.get("gap_normal_min_sec", 50))
    gn_max = int(tinfo.get("gap_normal_max_sec", 70))
    bw_sec = int(tinfo.get("burst_window_sec", 120))
    gb_min = int(tinfo.get("gap_burst_min_sec", 5))
    gb_max = int(tinfo.get("gap_burst_max_sec", 8))

    # Local attachment columns (optional)
    tpl_local1 = (tinfo.get("local_file_1") or "").strip()
    tpl_local2 = (tinfo.get("local_file_2") or "").strip()

    # Track in JOBS (your existing status map)
    JOBS[job_id] = {"status": "queued", "attempts": 0, "last_result": None}

    # --------------------------
    # Helpers: state + rotation
    # --------------------------
    def _normalize_allowed_sessions(val: str) -> list[str]:
        s = (val or "").strip()
        if not s or s.upper() == "ALL":
            return [x.get("senderId") for x in (FALLBACK_SESSIONS or []) if x.get("senderId")]
        return [p.strip() for p in s.split(",") if p.strip()]
    ...
    def _get_next_ts(st: dict, key: str) -> float:
        m = st.get("session_next_ts", {}) or {}
        try:
            return float(m.get(str(key), 0))
        except Exception:
            return 0.0

    def _set_next_ts(st: dict, key: str, ts: float) -> None:
        m = st.get("session_next_ts", {}) or {}
        m[str(key)] = float(ts)
        st["session_next_ts"] = m

    def _mark_last(st: dict, number: str, sender_key: str) -> None:
        last = st.get("last_by_number", {}) or {}
        last[str(number)] = {"ts": time.time(), "sender": str(sender_key)}
        st["last_by_number"] = last

    def _pick_round_robin_sender(st: dict, allowed: list[str]) -> str | None:
        if not allowed:
            return None
        idx = int(st.get("rr_index") or 0)
        sid = allowed[idx % len(allowed)]
    ...
    def _is_sender_on_cooldown(st: dict, key: str) -> bool:
        return time.time() < _get_next_ts(st, key)

    def _gap_seconds(is_burst: bool) -> int:
        if is_burst:
            return random.randint(gb_min, gb_max)
        return random.randint(gn_min, gn_max)

    # --------------------------
    # BSP enqueue (existing queue)
    # --------------------------
    ...
    # --------------------------
    # Fallback send (rotation)
    # --------------------------
    def _send_via_fallback():
        try:
            if not _fallback_can_send():
                app.logger.error("[queue] fallback not configured!")
                JOBS[job_id]["status"] = "failed_fallback_not_configured"
                return

            allowed = _normalize_allowed_sessions(allowed_sessions_raw)
            if not allowed:
                app.logger.error("[queue→fallback] allowed_sessions empty (no fallback sender available)")
                JOBS[job_id]["status"] = "failed_no_sender_available"
                return

            # burst check (same number within window) -> use same sender if possible
            now = time.time()

            with SEND_STATE_LOCK:
                st = _load_state()
                last = (st.get("last_by_number", {}) or {}).get(str(to), {}) or {}
                last_ts = float(last.get("ts") or 0)
                last_sender = last.get("sender")  # "BSP" or fallback senderId

                is_burst = bool(last_sender and last_sender != "BSP" and (now - last_ts) <= bw_sec)

                # choose sender
                sender_id = None
                if is_burst and str(last_sender) in allowed:
                    sender_id = str(last_sender)
                else:
                    sender_id = _pick_round_robin_sender(st, allowed)
                ...
                # cooldown check: if chosen sender is busy, try next ones quickly
                # (immediate fallback; no waiting)
                tries = 0
                while tries < len(allowed) and _is_sender_on_cooldown(st, sender_id):
                    tries += 1
                    sender_id = _pick_round_robin_sender(st, allowed)

                if _is_sender_on_cooldown(st, sender_id):
                    # all senders on cooldown -> mark as queued and let caller retry later if needed
                    JOBS[job_id]["status"] = "queued_fallback_all_cooldown"
                    _save_state(st)
                    return

                gap = _gap_seconds(is_burst)

                sess = get_fallback_session(sender_id)
                if not sess:
                    JOBS[job_id]["status"] = "failed_sender_not_in_env"
                    _save_state(st)
                    return

                app.logger.info(f"[queue→fallback] job_id={job_id} to={to} tpl={template_name} sender={sender_id}")

                # wrapper that may add extra attachments (your existing logic)
                sc, body = send_fallback_with_sales_extras(
                    str(to),
                    fb_text,
                    ...
                    template_name=template_name,
                    session=sess,  # ✅ IMPORTANT: make wrapper pass it to send_fallback_botmaster
                )
                ...
                if 200 <= sc < 300:
                    ...
                    # Save state: last sender + cooldown for this sender
                    _mark_last(st, str(to), sender_id)
                    _set_next_ts(st, sender_id, time.time() + gap)
                    _save_state(st)

                    app.logger.info(f"[queue→fallback] ✅ delivered via fallback for job_id={job_id} gap={gap}s")
                else:
    ...
    # --------------------------
    # Decision: BSP vs Fallback
    # --------------------------
    def _dispatch():
        # If global forced fallback -> always fallback
        if GLOBAL_FORCE_FALLBACK_ALL:
            return _send_via_fallback()

        # If template BSP is OFF -> fallback immediately
        if not tpl_bsp_enable:
            return _send_via_fallback()

        # If BSP is ON, we try BSP first BUT:
        # - if BSP is on cooldown -> fallback immediately (your requirement)
        # - if BSP cannot send -> fallback immediately
        with SEND_STATE_LOCK:
            st = _load_state()
            bsp_on_cooldown = _is_sender_on_cooldown(st, "BSP")

        # BSP config present?
        api_url = (os.getenv("WHATSAPP_API_URL", "") or "").strip()
        api_key = (os.getenv("WHATSAPP_KEY", "") or "").strip()
        ...
        # Otherwise put to BSP queue and set BSP cooldown (normal)
        gap = random.randint(gn_min, gn_max)
        _put_to_bsp_queue()

        with SEND_STATE_LOCK:
            st = _load_state()
            _mark_last(st, str(to), "BSP")
            _set_next_ts(st, "BSP", time.time() + gap)
            _save_state(st)

        app.logger.info(f"[queue→bsp] job_id={job_id} to={to} tpl={template_name} gap={gap}s")

    # Apply delay_seconds if provided (kept)
    if delay_seconds and delay_seconds > 0:
        threading.Timer(delay_seconds, _dispatch).start()
        app.logger.info(f"[queue] job scheduled id={job_id} delay={delay_seconds}s tpl={template_name}")
    else:
        _dispatch()
        app.logger.info(f"[queue] job executed id={job_id} tpl={template_name}")
...
def _bsp_can_send_now(tpl_name: str) -> bool:
    # BSP allowed only if Excel says ON for this template
    if not is_bsp_enabled_for_template(tpl_name):
        return False
    # must also have API url/key
    API_URL = (os.getenv("WHATSAPP_API_URL", "") or "").strip()
    KEY = (os.getenv("WHATSAPP_KEY", "") or "").strip()
    return bool(API_URL and KEY)

def _job_list_pending() -> list[str]:
    files = []
    for fn in os.listdir(OUTBOX_DIR):
        if fn.endswith(".json") and not fn.startswith("_"):
            files.append(os.path.join(OUTBOX_DIR, fn))
    files.sort()
    return files

def _job_move(path: str, dst_dir: str) -> None:
    os.makedirs(dst_dir, exist_ok=True)
    os.replace(path, os.path.join(dst_dir, os.path.basename(path)))

def _send_via_fallback_session(job: dict, session: dict) -> tuple[bool, str]:
    to = job["to"]
    tpl = job["template_name"]
    text = job.get("fallback_text") or job.get("raw_text") or ""

    # attachments from local_file_1/2 (optional)
    local_paths = []
    for k in ("local_file_1", "local_file_2"):
        p = (job.get(k) or "").strip()
        if p and os.path.exists(p):
            local_paths.append(p)

    # 1) main send
    sc, body = send_fallback_botmaster(to, text, media_url=None, local_path=(job.get("pdf_path") or None), session=session)
    if not (200 <= sc < 300):
        return False, f"fallback_failed_{sc}"

    # 2) extra local files (if specified)
    for lp in local_paths:
        time.sleep(1)
        sc2, body2 = send_fallback_botmaster(to, "", media_url=None, local_path=lp, session=session)
        if not (200 <= sc2 < 300):
            return False, f"fallback_attach_failed_{sc2}"

    return True, "ok"

def outbox_worker_loop():
    app.logger.info("[outbox] worker started")
    while True:
        try:
            files = _job_list_pending()
            if not files:
                time.sleep(0.5)
                continue
            ...
            for path in files:
                ...
                to = job.get("to", "")
                tpl = (job.get("template_name") or "").strip().lower()
                now = time.time()

                # burst vs normal timing
                bw = int(job.get("burst_window_sec", 120))
                gb_min = int(job.get("gap_burst_min_sec", 5))
                gb_max = int(job.get("gap_burst_max_sec", 8))
                gn_min = int(job.get("gap_normal_min_sec", 50))
                gn_max = int(job.get("gap_normal_max_sec", 70))

                # check last sender for burst
                with SEND_STATE_LOCK:
                    st = _load_send_state()
                    last = (st.get("last_by_number", {}) or {}).get(str(to), {}) or {}
                    last_ts = float(last.get("ts") or 0)
                    last_sender = last.get("sender")  # "BSP" or senderId

                burst = bool(last_sender and (now - last_ts) <= bw)
                gap = random.randint(gb_min, gb_max) if burst else random.randint(gn_min, gn_max)

                # ------------- BSP FIRST (only if template bsp_enable=ON) -------------
                # If BSP is on cooldown → USE FALLBACK IMMEDIATELY (your requirement)
                bsp_key = "BSP"
                bsp_on_cooldown = now < _get_next_ts(bsp_key)

                sent = False
                why = ""

                if last_sender == "BSP" and burst:
                    # burst prefers same sender
                    if _bsp_can_send_now(tpl) and not bsp_on_cooldown:
                        # Use your existing BSP send flow:
                        # IMPORTANT: you already have BSP template send in your code (keep it).
                        # Here we just enqueue into your existing JobQueue (so your current BSP logic stays).
                        JobQueue.put(job)
                        _mark_last_sent(to, "BSP")
                        _set_next_ts("BSP", time.time() + gap)
                        sent = True
                        why = "queued_bsp_burst"
                    # else: fallback immediately below

                if not sent and _bsp_can_send_now(tpl) and (not bsp_on_cooldown) and (not burst):
                    # normal BSP try
                    JobQueue.put(job)
                    _mark_last_sent(to, "BSP")
                    _set_next_ts("BSP", time.time() + gap)
                ...
                if not sent:
                    # ------------- FALLBACK ROTATION -------------
                    allowed = _normalize_allowed_sessions(job.get("allowed_sessions", "ALL"))
                    sender_id = None

                    # burst wants same fallback sender
                    if burst and last_sender and str(last_sender) != "BSP" and str(last_sender) in allowed:
                        sender_id = str(last_sender)
                    else:
                        sender_id = _pick_round_robin_sender(allowed)

                    if not sender_id:
                        job["attempts"] = int(job.get("attempts") or 0) + 1
                        job["last_error"] = "no_sender_available"
                        with open(path, "w", encoding="utf-8") as f:
                            json.dump(job, f, ensure_ascii=False, indent=2)
                        continue

                    # per-sender cooldown
                    if now < _get_next_ts(sender_id):
                        # cannot send with this sender now → skip file and check next
                        continue

                    sess = get_fallback_session(sender_id)
                    if not sess:
                        job["attempts"] = int(job.get("attempts") or 0) + 1
                        job["last_error"] = "sender_not_in_env"
                        with open(path, "w", encoding="utf-8") as f:
                            json.dump(job, f, ensure_ascii=False, indent=2)
                        continue

                    ok, err = _send_via_fallback_session(job, sess)
                    if ok:
                        _mark_last_sent(to, sender_id)
                        _set_next_ts(sender_id, time.time() + gap)
                        sent = True
                        why = f"sent_fallback_{sender_id}"
                    else:
                        job["attempts"] = int(job.get("attempts") or 0) + 1
                        job["last_error"] = err
                        with open(path, "w", encoding="utf-8") as f:
                        ...
                        # after 3 attempts → move to failed
                        if job["attempts"] >= 3:
                            _job_move(path, OUTBOX_FAIL_DIR)
                        continue

                # if sent/queued ok → move to _sent
                if sent:
                    app.logger.info(f"[outbox] ✅ {why} file={os.path.basename(path)}")
                    _job_move(path, OUTBOX_SENT_DIR)

            time.sleep(0.2)

        except Exception:
            app.logger.exception("[outbox] worker crashed")
            time.sleep(1)
